...
def edit_extract(yamldata):    # edits extract/Extract.py
    ...
    g.write("Ts = {0}\n".format(Ts))
    g.write("Data = {0}\n".format(Data))
    g.write("negative = [] # list and corrs to make -ve\n")
    g.write("tneg = [] # list and corrs to make -ve for odd t\n")
    g.write("Tneg = [] # list and corrs to make -ve for odd T these three effects are cumulative\n")
    g.write("##########################################\n")
    g.write("\n")
    g.write("\n")
    g.write("\n")
    ...
    g.write("\n")
    g.write("negtag = 'neg'\n")
    g.write("for element in negative:\n")
    g.write("    negtag ='{0}{1}'.format(negtag,element)\n")
    g.write("gpl = '{0}{1}_{2}cfgs_{3}.gpl'.format(tag,labforgpl,int((cfgend-cfgstart)/dconf +1-len(skip)),negtag)\n")
    ...
    if 'parent prop' in yamldata:
        ...
        g.write("                            average_imaginary(filenames,datatag,g,neg,'{0}')\n".format(yamldata['parent prop']['name']))
        g.write("                        else:\n")
        g.write("                            average(filenames,datatag,g,neg,'{0}')\n".format(yamldata['parent prop']['name']))        
    if 'daughter prop' in yamldata:
    ...
    if yamldata['lattice info']['justtwopoints'] == False:
        for threeptlabel in yamldata['three points']['label']:
            ...
            g.write("                            if '{0}' in imaginary:\n".format(threeptlabel))
            g.write("                                average_imaginary(filenames,datatag,g,neg,'{0}',2)\n".format(threeptlabel))
            g.write("                            else:\n")
            g.write("                                average(filenames,datatag,g,neg,'{0}',2)\n".format(threeptlabel)) 
    ...
    g.write("            if tag in tneg and src%2 == 1:\n")
    g.write("                result[filename].append(-1*float(lines[j].split()[1]))\n")
    g.write("            else:\n")
    g.write("                result[filename].append(lines[j].split()[1])\n")
    g.write("        if '0.000000e+00' in result[filename]:\n")
    ...
    g.write("\n")
    g.write("\n")
    g.write("def average_imaginary(filenames,datatag,g,neg,tag,fac=1):\n")
    g.write("    result = collections.OrderedDict()   \n")
    g.write("    for filename in filenames:                          #filenames is list of files to be averaged\n")
    g.write("        src = int(filename.split('/')[-1].split('_t')[1].split('_')[0])\n")
    g.write("        result[filename] = []\n")
    ...
    g.write("        for j in range(lastdots+1,lastdots+1+nt):\n")
    g.write("            if tag in tneg and src%2 == 1:\n")
    g.write("                result[filename].append(-1*float(lines[j].split()[2]))\n")
    g.write("            else:\n")
