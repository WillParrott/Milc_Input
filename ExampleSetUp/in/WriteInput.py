...
def edit_extract(yamldata):    # edits extract/Extract.py
    ...
    if 'parent prop' in yamldata:
        for st in yamldata['parent prop']['spin_taste']:
            data.append('./corrs/{0}-{1}_tw{2}'.format(yamldata['parent prop']['name'],st,yamldata['parent prop']['twist']))
    if 'daughter prop' in data:
        for st in yamldata['daughter prop']['spin_taste']:
            for twist in yamldata['daughter prop']['twists']:
                data.append('./corrs/{0}-{1}_tw{2}'.format(yamldata['daughter prop']['name'],st,twist))
    if yamldata['lattice info']['justtwopoints'] == False:
    ...
    if 'parent prop' in yamldata:
        ...
        g.write("                    for Mass in Massess:\n")
        g.write("                        mass = masses[Mass]\n")
        g.write("                        spin_tast = element.split('/')[2].split('_')[1]\n")
        g.write("                        filenames = []\n")
    ...
    if 'daughter prop' in yamldata:
        ...
        g.write("            for element in data:\n")
        g.write("                if element.split('/')[2][0] == '{0}':\n".format(yamldata['daughter prop']['name']))
        g.write("                    for Twist in Twists:\n")
        g.write("                        twist = twists[Twist]\n")
        g.write("                        spin_tast = element.split('/')[2].split('_')[1]\n")
        g.write("                        filenames = []\n")
    ...
    if yamldata['lattice info']['justtwopoints'] == False:
        for threeptlabel in yamldata['three points']['label']:
            g.write("        if '{0}' in Data:\n".format(threeptlabel))
            g.write("            for element in data:\n")
            g.write("                if element.split('/')[2].split('-')[1] == '{0}':\n".format(threeptlabel))
            g.write("                    for Twist in Twists:\n")
            g.write("                        twist = twists[Twist]\n")
            ...
            g.write("                            for T in Ts:\n")
            g.write("                                filenames = []\n")
            g.write("                                datatag = '{0}_T{1}_m{2}_m{3}_m{4}_tw{5}'.format(threepointlabel,T,mdaughter,mass,mspec,twist)\n")
            g.write("                                for source in sources:\n")
            g.write("                                    filenames.append('{0}/{10}current-{1}.{3}.{4}_t{5}_T{6}_m{7}_m{8}_m{9}_tw{2}'.format(element,element.split('/')[2].split('-')[1],twist,lat,cfg,source,T,mdaughter,mass,mspec,tag)\n")
            g.write("                                average(filenames,datatag)\n")
    ...
    g.write("\n")
    g.write("    \n")
    g.write("def getfilenames(t0s,cfg,):\n")
    g.write("    \n")
    g.write("    \n")
    g.write("    return(filenames,datatag)\n")
    g.write("    \n")
